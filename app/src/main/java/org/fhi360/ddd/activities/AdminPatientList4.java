...
public class AdminPatientList4 extends AppCompatActivity implements SearchView.OnQueryTextListener, MenuItem.OnActionExpandListener {
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        patientRecyclerAdapter = new PatientAdpter3(filteredValues, AdminPatientList4.this);
    ...
    public void resetSearch() {
        patientRecyclerAdapter = new PatientAdpter3(listPatients, AdminPatientList4.this);
        recyclerViewHts.setAdapter(patientRecyclerAdapter);
    ...
    private void getPatient() {
        mPb = new ProgressDialog(AdminPatientList4.this);
        mPb.setProgress(0);
        ...
        Call<Data> objectCall = clientAPI.getAllPatientsFacilityId(DDDDb.getInstance(AdminPatientList4.this).facilityRepository().getFacility().getId());//downLoad(deviceId, pin, accountUserName, accountPassword);
        objectCall.enqueue(new Callback<Data>() {
            ...
            public void onResponse(@NonNull Call<Data> call, @NonNull retrofit2.Response<Data> response) {
                if (response.isSuccessful()) {
                    ...
                    for (PatientDto patientDto : listPatients) {
                        ...
                        Patient patient1 = DDDDb.getInstance(AdminPatientList4.this).patientRepository().findOne(patientDto.getId());
                        if (patient1 != null) {
                            patient1.setOutLetName(patientDto.getOutLetName());
                            DDDDb.getInstance(AdminPatientList4.this).patientRepository().update(patient1);
                            mPb.hide();
                        } else {
                            DDDDb.getInstance(AdminPatientList4.this).patientRepository().save(patient);
                    ...
                    patientRecyclerAdapter = new PatientAdpter3(listPatients, AdminPatientList4.this);
                    RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(AdminPatientList4.this);
                    ...
                    FancyToast.makeText(AdminPatientList4.this, "DDD ne peut pas obtenir les dossiers des patients", FancyToast.LENGTH_LONG, FancyToast.ERROR, false).show();
                    mPb.hide();
                }
            ...
            public void onFailure(@NonNull Call<Data> call, @NonNull Throwable t) {
                t.printStackTrace();
                FancyToast.makeText(AdminPatientList4.this, "Pas de connexion Internet", FancyToast.LENGTH_LONG, FancyToast.ERROR, false).show();
                mPb.hide();
            }
